...
### Timed events: start/end time parsing (avoid accidental all-day events)

`khal new` is **positional** and parses times based on your configured formats.
If it can't parse a time, it may treat it as part of the **title** and fall back to a **date-only DTSTART** (an all-day event).

Quick help:
```bash
khal new --help
khal printformats   # shows the date/time/datetime formats khal will accept
```

Examples (12-hour clock):
```bash
# today 6pm–7pm
khal new today 6:00 PM 7:00 PM "Test event"

# explicit date 6pm–7pm
khal new 2026-02-03 6:00 PM 7:00 PM "Test event"

# duration instead of end time (1 hour)
khal new today 6:00 PM 1h "Test event"

# pick a specific calendar
khal new today 6:00 PM 7:00 PM "Test event" -a Family
```

If you prefer to display times in **12h** format, set in `~/.config/khal/config`:
```ini
[locale]
timeformat = %I:%M %p
```
